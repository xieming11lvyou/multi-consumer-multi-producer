...
//      {
  ...
  //{
    ...
    p1->RemovePacketTag(LastHopTag);
    Ptr<MobilityModel> mobility = m_node->GetObject<MobilityModel> ();
    LastHopTag.SetMobility((mobility->GetPosition()).x);
    p1->AddPacketTag (LastHopTag);
    velocityTag.SetVelocity((mobility->GetVelocity()).x);
                           ...
                           NetDevice::PacketType packetType)
{
  ...
  LastHopTag LastHopTag;
  packet->PeekPacketTag(LastHopTag);
  ...
  //cout<<"At: "<<Simulator::Now()<<" from node:"<<node->GetId()<<" location:"<<LastHopTag.GetMobility()<<endl;

  // cout<<"At: "<<Simulator::Now()<<" node:"<<node->GetId()
  //     <<" location:"<<LastHopTag.GetMobility()
  //     <<" velocity:"<<velocityTag.GetVelocity()<<endl;
  SourceDestTag destTag;
  if(packet->PeekPacketTag(destTag))
      {
  if(abs(m_node->GetObject<MobilityModel>()->GetPosition().x-destTag.GetSource()) <=
      abs(LastHopTag.GetMobility()-destTag.GetSource()) &&
  ...
  if(2010>=m_node->GetObject<MobilityModel>()->GetPosition().x &&
      m_node->GetObject<MobilityModel>()->GetPosition().x>=destTag.GetSource())
    {
    m_isbetween = true;
    }
  else
  ...
  distance = GetDistance(LastHopTag.GetMobility());
