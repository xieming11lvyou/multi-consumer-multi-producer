...
//  cout<<"Index"<<indexTag.GetIndex()<<endl;
  ...
  PCTag consumerTag;
  consumerTag.SetFlag(2);//2->consumer
  interest->GetPayload ()->AddPacketTag (consumerTag);

  SourceDestTag destTag;
  destTag.SetDest(5000);//destination coor(0,2000)
  destTag.SetSource(m_node->GetObject<MobilityModel>()->GetPosition().x);
  interest->GetPayload ()->AddPacketTag (destTag);


  cout<<"Retans"<<"\t"<<Simulator::Now().GetSeconds()<<endl;
  m_transmittedInterests (interest, this, m_face);
  m_face->ReceiveInterest (interest);
  if (!m_reSendEvent.IsRunning())
    m_reSendEvent = Simulator::Schedule (Seconds(10.0), &MyConsumer::RertransmitPacket, this);
}
...
//-----------------------
  ...
  cout<<m_node->GetId()<<" Receive"<<"\t"<<t2<<"\t"<<"delay"<<"\t"<<(t2-t1)<<"\t";
  m_numData++;
...
//  cout<<endl;
  ...
  Ptr<Name> nameWithSequence = Create<Name> (data->GetName ());
  //
  ...
  AllTag ackTag;
  ackTag.SetACK(1);//1->ackflag
  interest->GetPayload ()->AddPacketTag(ackTag);
  m_face->ReceiveInterest (interest);
...
if(m_numData<1000)
  {
  Simulator::Schedule (Seconds(1.0), &MyConsumer::ScheduleNextPacket, this);
  }
